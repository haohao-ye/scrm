...
<el-card class="mt-15">
    ...
    <el-table v-loading="loading" :data="deptList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="部门id" align="center" prop="deptNo" />
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const deptNo = row.deptNo || this.ids
      getDept(deptNo).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const deptNos = row.deptNo || this.ids;
      this.$confirm('是否确认删除部门编号为"' + deptNos + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delDept(deptNos);
        }).then(() => {
          this.getList();
          this.$message("删除成功");
        })
